...
public class ClientOut implements Runnable {
	...
	public void run() {
		...
		try {
			Message message = new Message(playerID, pList.get(playerID).R,
			...
			while (!Thread.interrupted()) {
				if (first_run) {
					player_info_message = new Message(playerID, null,999,0,0);
					first_run = false;
					out.writeObject(player_info_message);
					out.reset();
				} else {
					message.putValues(playerID, pList.get(playerID).R,pList.get(playerID).colorComponent.getR(), pList.get(playerID).colorComponent.getG(),
							pList.get(playerID).colorComponent.getB());
